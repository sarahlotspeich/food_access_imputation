...
# --------------------------------------------------------------------
sim_data = function(N, beta0 = fix_beta0, beta1 = fix_beta1, muU = fix_muU, sigmaU) {
...
for (N in c(390, 2200)) {
  ...
  # And error standard deviation: 0.1, 0.2, 0.4, 0.8
  for (sigma in c(0.1, 0.2, 0.4, 0.8)){
    # Be reproducible
    ...
    sett_res = data.frame(sim = paste(sim_seed, 1:num_reps, sep = "-"), 
                          N, beta0 = fix_beta0, beta1 = fix_beta1, muU = fix_muU, sigmaU = sigma, pV = fix_pV, avg_prev = NA, ## simulation setting
...
# Timing from tictoc:
## Sims with N = 390: 168.916 sec elapsed
## Sims with N = 2200: 437.192 sec elapsed