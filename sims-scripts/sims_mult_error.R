...
## maxW = max of the measurement error distribution
# --------------------------------------------------------------------
sim_data = function(N, avg_prev = fix_avg_prev, beta1 = fix_beta1, maxW) {
  ## Get model intercept
  beta0 = log(avg_prev)
...
for (N in c(100, 340, 2200)) {
  # And max of multiplicative error: 1.25, 1.4, 1.6, 2
  for (m in c(1.25, 1.4, 1.6, 2)) {
    # Be reproducible
    ...
    sett_res = data.frame(sim = paste(sim_seed, 1:num_reps, sep = "-"), 
                          N, beta1 = fix_beta1, maxW = m, pV = fix_pV, avg_prev = NA, ## simulation setting
    ...
    for (r in 1:num_reps) {
      ...
      dat = sim_data(N  = N, ## sample size
                     maxW = m) ## max of multiplicative error
      ...
      write.csv(x = sett_res,
                file = paste0("mult_error/proximity_N", N, "_maxW", 10 * m, "_seed", sim_seed, ".csv"), 
                row.names = F)
    }
  }
}
