...
fix_N = 340 ## number of neighborhoods (sample size)
...
# --------------------------------------------------------------------
sim_data = function(N = fix_N, avg_prev, beta1, muU = fix_muU, sigmaU = fix_sigmaU) {
  ## Get model intercept
  beta0 = log(avg_prev)
...
# Loop over different average prevalences: 7%, 11%, and 35%
for (alpha in c(0.07, 0.11, 0.35)) {
  # And prevalence ratios: log(0.95),  log(0.99), log(1), log(1.01), and log(1.05)
  for (pr in c(0.95, 0.99, 1, 1.01, 1.05)){
    # Be reproducible
    ...
    sett_res = data.frame(sim = paste(sim_seed, 1:num_reps, sep = "-"), 
                          N = fix_N, beta0 = log(alpha), beta1 = log(pr), muU = fix_muU, sigmaU = fix_sigmaU, pV = fix_pV, avg_prev = NA, ## simulation setting
    ...
    for (r in 1:num_reps) {
      # Generate data
      dat = sim_data(avg_prev = alpha, ## average prevalence
                     beta1 = log(pr)) ## log prevalence ratio
      ...
      write.csv(x = sett_res,
                file = paste0("vary_prev/proximity_prev", alpha * 100, "_pr", 100 * pr, "_seed", sim_seed, ".csv"), 
                row.names = F)
    }
  }
}
