...
################################################################################
# Load healthy foods stores for Forsyth County, NC and the surrounding counties from the historic SNAP retailers data
grocery = read.csv(file = "https://raw.githubusercontent.com/sarahlotspeich/food/main/forsyth-data/healthy_foods_stores_2022.csv")
nrow(grocery) ## M = 525 healthy foods stores

# Create data frame to fill with food access variables
food_access = neighborhoods |> 
  dplyr::select(LocationID, LONGITUDE, LATITUDE) |> 
  dplyr::mutate(dist_closest_straight = NA, ## Straight-line distance to nearest healthy foods store
...
for (i in 1:nrow(food_access)) {
  ...
  temp = get_centroid_address(longitude = neighborhoods$LONGITUDE[i], 
  ...
  if (!any(is.na(temp))) {
    ...
    ## Save data
    food_access |> 
      write.csv(file = "closest_healthy_foods_store_2022.csv", 
                row.names = FALSE)
  }
}
