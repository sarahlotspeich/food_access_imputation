...
res = read.csv(file = "https://raw.githubusercontent.com/sarahlotspeich/food/main/sims-data/vary_sigmaU_sims_combined.csv")
...
## Change column names 
colnames(res_summ) = c("$\\pmb{N}$", "$\\pmb{\\sigma_U}$", rep(c("Bias", "ESE", "ASE", "CP"), times = 4))
res_summ |> 
  kable(format = "latex", 
        booktabs = TRUE, 
        escape = FALSE, 
...
## Note: For visual reasons, the \addlinespace were manually moved in LaTex
