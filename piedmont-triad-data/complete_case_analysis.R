...
### Non-Spatial Models
mod_diab = glm(formula = Y_DIABETES ~ METRO * X_partial, 
               family = poisson(link = "log"), 
               offset = log(O_POP),
               data = food_access)
mod_obes = glm(formula = Y_OBESITY ~ METRO * X_partial,
               family = poisson(link = "log"), 
               offset = log(O_POP),
               data = food_access)
...
mod_diab = fitme(formula = Y_DIABETES ~ METRO * X_partial + adjacency(1 | GEOID) + offset(log(O_POP)), 
                 family = poisson(link = "log"), 
                 adjMatrix = ptW,
                 data = food_access)
...
## Predictor X = proximity to healthy foods based on map-based distance
mod_obes = fitme(formula = Y_OBESITY ~ METRO * X_partial + adjacency(1 | GEOID) + offset(log(O_POP)), 
                 family = poisson(link = "log"), 
                 adjMatrix = ptW,
                 data = food_access)

# Save results for forest plot
cc_res = do.call(what = dplyr::bind_rows, 
